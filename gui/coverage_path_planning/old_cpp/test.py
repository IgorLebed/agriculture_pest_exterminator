...
def search_free_grid_index_at_edge_y(grid_map, from_upper=True):
    y_index = None
    x_indexes = []
...
def setup_grid_map(ox, oy, resolution, sweep_direction, offset_grid=10):
    ...
    #grid_map.set_value_from_polygon([35, 50, 50 ,35, 35], [40,40,80,80, 40], 1.0, inside=True)
    #grid_map.expand_grid()
    x_inds_goal_y = []
    goal_y = 0
...
def setup_grid_map_with_expand(ox, oy, resolution, sweep_direction, offset_grid=10):
    width = math.ceil((max(ox) - min(ox)) / resolution) + offset_grid
    height = math.ceil((max(oy) - min(oy)) / resolution) + offset_grid
    center_x = (np.max(ox) + np.min(ox)) / 2.0
    center_y = (np.max(oy) + np.min(oy)) / 2.0
...
def planning(ox, oy, resolution,
    ...
    grid_map, x_inds_goal_y, goal_y = setup_grid_map(rox, roy, resolution,
                                                     sweeping_direction)
    #grid_map.set_value_from_polygon([10, 20, 20 ,10, 10], [30,30,10,10, 30], 1.0, inside=True)
...
def planning_expand(ox, oy, resolution,
             moving_direction=SweepSearcher.MovingDirection.RIGHT,
             sweeping_direction=SweepSearcher.SweepDirection.UP,
             ):
    sweep_vec, sweep_start_position = find_sweep_direction_and_start_position(
    ...
    grid_map, x_inds_goal_y, goal_y = setup_grid_map_with_expand(rox, roy, resolution,
                                                     sweeping_direction)
    #grid_map.set_value_from_polygon([10, 20, 20 ,10, 10], [30,30,10,10, 30], 1.0, inside=True)
...
def planning_animation(ox, oy, resolution):  # pragma: no cover
    px_1, py_1 = planning(ox, oy, resolution)
    px_2, py_2 = planning_expand(ox, oy, resolution)
    arr_1 = list(map(list,zip(px_1,py_1)))
    arr_2 = list(map(list,zip(px_2,py_2)))
    array = list(map(list, set(map(tuple, arr_2)) - set(map(tuple, arr_1))))
    new_array = [x for x in arr_2 if x not in arr_1]
    print(new_array)
    #print(px_1)
    #print(px_2)
    #list12 = extendList(arr_1, arr_2)
    #print(list12)
    # animation
    if do_animation:
        for ipx, ipy in zip(px_1, py_1):
            plt.cla()
            # for stopping simulation with the esc key.
            plt.gcf().canvas.mpl_connect(
                ...
                lambda event: [exit(0) if event.key == 'escape' else None])
            plt.plot(ox, oy, "-xb")
            plt.plot(px_1, py_1, "-r")
            plt.plot(ipx, ipy, "or")
            plt.axis("equal")
            plt.grid(True)
            plt.pause(0.1)
        for ipx, ipy in zip(px_2, py_2):
            plt.cla()
            # for stopping simulation with the esc key.
            plt.gcf().canvas.mpl_connect(
                'key_release_event',
...
def main():  # pragma: no cover
    ...
    ox = [10.0, 100.0, 100.0, 40.0, 10.0, 10.0]
    oy = [10.0, 20.0, 100.0, 150.0, 120.0, 10]
    resolution = 5

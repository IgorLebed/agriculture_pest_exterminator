...
def get_angle(p_t0, p_t1, p_want):
    ...
    def distance(a, b):
        # Vincenty formula
        delta_lambda = a[1] - b[1]
        delta_phi = a[0] - b[0]
        """ delta_sigma = (sqrt((cos(b[1])*sin(delta_lambda))**2 + (cos(a[0])*sin(b[0]) - sin(a[0])*cos(b[0])*cos(delta_lambda))**2)
                       / (sin(a[0])*sin(b[0]) + cos(a[0])*cos(b[0])*cos(delta_lambda))) """
        delta_sigma = 2 * asin(sqrt(sin(delta_phi/2)**2 + (1 - sin(delta_phi/2)**2 - sin((a[0] + b[0])/2)**2)*sin(delta_lambda/2)**2))
        # Distance
        r = 6371.009  # km
...
if __name__ == '__main__':
    # Вверх и вправо
    print(get_angle((59.947365, 30.303307),
                    (59.947354, 30.300042),
                    (59.947016, 30.303308)))
    # Наоборот
    print(get_angle((59.935954, 30.285555),
                    (59.936062, 30.285555),
                    (59.935954, 30.285247)))
    ...
    # ~45
    print(get_angle((59.946213, 30.271605),
                    (59.942405, 30.275963),
                    (59.940679, 30.267856)))

    # Совпадения
    print(get_angle((59.916990, 30.337972),
                    (59.916990, 30.337972),
                    (59.916990, 30.337972)))
